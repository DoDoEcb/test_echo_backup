...
require_once("logicas.php");
    ...
    $linkUm = ""; $linkDois = "active"; $linkTres = ""; $linkQuatro = ""; $linkCinco = "";
                        ...
                        <div class="widget-content nopadding">
                            <table class="table table-bordered table-striped">
                              ...
                              if(isset($_POST['buscar'])){
                                   $busca = $_POST['search'];
                                  $sql = $pdo->pdo->query("SELECT *,f.username as username FROM usuarios as u INNER JOIN  financeiro as f INNER JOIN renovacao as r  on u.id=f.personal_id AND r.personal_id=u.id WHERE f.status='Ativo' AND f.block='Desbloqueado' AND f.username LIKE '%$busca%'");

                              }else {
                                  $sql = $pdo->pdo->query("SELECT * FROM usuarios as u INNER JOIN  financeiro as f INNER JOIN  renovacao as r  on u.id=f.personal_id AND r.personal_id=u.id WHERE f.status='Ativo' AND f.block='Desbloqueado' LIMIT 100");
                              }
                                if($sql->rowCount()>0){
