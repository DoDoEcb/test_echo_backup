...
$DolartoBtc =  currency('BTC', 'BRL', 1); // 1 bitcoin convertido para Dolar
 $BtctoDolar =  currency('BRL', 'BTC'); // valor do dólar
            ...
            <div class="widget-box">
                ...
                <div class="widget-content nopadding">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <?php
                        $sql = $pdo->pdo->query("SELECT * FROM invoice_upgrade  as i INNER JOIN usuarios as u on i.personal_id=u.id WHERE i.descricao LIKE '%Renewal invoice%' AND i.status!='Payd'");
                        if($sql->rowCount()>0){
                            ...
                            foreach($res as $row) {
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td> <?php echo $row->price_now?> BTC</td>
                                    ...
                                    <td class="center">
                                        <a href="renovacao?confirm=<?php echo $row->pedido?>&username=<?php echo $row->username?>&&saldo=<?php echo $row->saldo_renew?>&&valor=<?php echo $row->price_now?>&&patrocinador=<?php echo $row->patrocinador?> "> <button class="btn btn-info">Confirmar</button></a>
